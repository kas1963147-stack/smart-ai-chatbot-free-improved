...
if (defined('SWC_CHATBOT_VERSION')) {
    add_action('admin_notices', function() {
        echo '<div class="notice notice-error"><p><strong>Quark AgentFlow AI:</strong> Another version of the Quark AgentFlow AI plugin is already active. Please deactivate the active version first to prevent conflicts.</p></div>';
    });
...
function swc_chatbot_load_chat_system() {
    ...
    if (file_exists(SWC_CHATBOT_PATH . 'src/History/HistorySchema.php')) {
        require_once SWC_CHATBOT_PATH . 'src/History/HistorySchema.php';
    }
    ...
    // Register Toolkits (agentic tools)
    add_action('init', function() {
        ...
        foreach ($toolkits as $folder => $class) {
            ...
            if (file_exists($file)) {
                require_once $file;
                $fullClass = "Quarksol\\AgentFlowAI\\Toolkits\\{$folder}\\{$class}";
