 ...
 * 
 * Affordable inference for open source models.
 */
...
class SWC_Chatbot_SiliconFlow_Provider extends SWC_Chatbot_AI_Provider {
    
    private $api_key;
    private $base_url = 'https://api.siliconflow.cn/v1';
    ...
    private static $models = array(
        ...
        'THUDM/glm-4-9b-chat' => array('id' => 'THUDM/glm-4-9b-chat', 'name' => 'GLM-4 9B', 'contextWindow' => 128000, 'maxOutputTokens' => 4096),
        'internlm/internlm2_5-20b-chat' => array('id' => 'internlm/internlm2_5-20b-chat', 'name' => 'InternLM 2.5 20B', 'contextWindow' => 32768, 'maxOutputTokens' => 8192)
    );
    
    public function __construct($api_key, $model = 'deepseek-ai/DeepSeek-V3', $temperature = 0.7, $max_tokens = null) {
    ...
    public function chat($message, $history = array()) {
        ...
        if ($status !== 200) return new WP_Error('sf_error', 'SiliconFlow error: ' . ($body['error']['message'] ?? 'Unknown'));
        return $body['choices'][0]['message']['content'] ?? new WP_Error('sf_error', 'Invalid response');
    }
