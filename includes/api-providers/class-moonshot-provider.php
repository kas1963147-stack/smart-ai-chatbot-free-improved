<?php
/**
 * Moonshot AI Provider Class (Kimi)
 * 
 * Chinese AI provider.
 */
...
class SWC_Chatbot_Moonshot_Provider extends SWC_Chatbot_AI_Provider {
    
    private $api_key;
    private $base_url = 'https://api.moonshot.cn/v1';
    
    private static $models = array(
        'moonshot-v1-128k' => array('id' => 'moonshot-v1-128k', 'name' => 'Moonshot 128K', 'contextWindow' => 128000, 'maxOutputTokens' => 8192),
        'moonshot-v1-32k' => array('id' => 'moonshot-v1-32k', 'name' => 'Moonshot 32K', 'contextWindow' => 32000, 'maxOutputTokens' => 8192),
        'moonshot-v1-8k' => array('id' => 'moonshot-v1-8k', 'name' => 'Moonshot 8K', 'contextWindow' => 8192, 'maxOutputTokens' => 4096)
    );
    
    public function __construct($api_key, $model = 'moonshot-v1-32k', $temperature = 0.7, $max_tokens = null) {
    ...
    public function get_model() {
        return isset(self::$models[$this->model_id]) ? self::$models[$this->model_id] : array(
            'id' => $this->model_id, 'name' => $this->model_id, 'contextWindow' => 32000, 'maxOutputTokens' => 8192
        );
    }
    ...
    public function chat($message, $history = array()) {
        ...
        if ($status !== 200) return new WP_Error('moonshot_error', 'Moonshot error: ' . ($body['error']['message'] ?? 'Unknown'));
        return $body['choices'][0]['message']['content'] ?? new WP_Error('moonshot_error', 'Invalid response');
    }
